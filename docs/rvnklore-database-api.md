...
- Schema Reference: [rvnklore-schema.md](rvnklore-schema.md)
- Database Patterns: [database-patterns.md](database-patterns.md)
- Architecture Refactor: [../architecture/rvnklore-database-refactor.md](../architecture/rvnklore-database-refactor.md)
...
RVNKLore implements automatic database reconnection to handle transient connection failures and ensure service continuity during startup and runtime.

### Startup Reconnection Flow

When a component (e.g., SubmissionManager) initializes before the database is ready:

```
Component Init → Database Check → Unavailable → Retry with Backoff → Connected
```

**Startup Log Pattern**:
```
[WARN] SubmissionManager: Database unavailable at initialization
[INFO] DatabaseHelper: Attempting reconnection (attempt 1/5)
[INFO] DatabaseHelper: Connection established after 2.3s
[INFO] SubmissionManager: Database connection restored
```

### DatabaseHelper Reconnection
...
public class DatabaseHelper {
    ...
    public CompletableFuture<Connection> getConnectionWithRetry() {
        return CompletableFuture.supplyAsync(() -> {
            int attempt = 0;
            ...
            while (attempt < MAX_RECONNECT_ATTEMPTS) {
                try {
                    ...
                    if (conn != null && conn.isValid(5)) {
                        if (attempt > 0) {
                            logger.info("Database reconnected after " + attempt + " attempts");
                        }
                        return conn;
                    }
                } catch (SQLException e) {
                    attempt++;
                    logger.warning("Database connection attempt " + attempt + 
                        "/" + MAX_RECONNECT_ATTEMPTS + " failed: " + e.getMessage());
            ...
            throw new RuntimeException("Failed to reconnect after " + 
    ...
    public void triggerReconnection() {
        ...
        reconnecting = true;
        logger.info("DatabaseHelper: Attempting reconnection");
        
        databaseManager.refreshConnectionPool()
            .thenRun(() -> {
                reconnecting = false;
                logger.info("DatabaseHelper: Connection pool refreshed");
                
                // Fire recovery event
                DatabaseRecoveryEvent event = new DatabaseRecoveryEvent();
                Bukkit.getPluginManager().callEvent(event);
            })
            .exceptionally(e -> {
                reconnecting = false;
                logger.error("DatabaseHelper: Reconnection failed", (Throwable) e);
                return null;
            });
    }
}
...
### Service Layer Recovery Pattern

Services using FallbackTracker integrate with reconnection:
...
public class SubmissionServiceImpl implements ISubmissionService {
    ...
    public SubmissionServiceImpl(DatabaseHelper dbHelper) {
        ...
        Bukkit.getPluginManager().registerEvents(new Listener() {
            @EventHandler
            public void onDatabaseRecovery(DatabaseRecoveryEvent event) {
                fallbackTracker.recordSuccess();
                logger.info("SubmissionService: Database connection restored");
            }
        }, plugin);
    }
...
| Scenario | Behavior | Timeout |
|----------|----------|---------|
| Startup unavailable | Retry with exponential backoff | 5 attempts (~15s) |
| Runtime connection lost | HikariCP automatic retry | Connection timeout (30s) |
| Pool exhaustion | Wait for available connection | Acquisition timeout (10s) |
| Manual trigger | Refresh connection pool | Immediate |

### Recovery Events

```java
// Fired when database connection is restored
public class DatabaseRecoveryEvent extends Event {
...
@EventHandler
public void onRecovery(DatabaseRecoveryEvent event) {
    logger.info("Database recovered after " + 
        event.getAttemptsTaken() + " attempts at " + 
        event.getRecoveryTime());
    
    // Re-initialize cached data
    refreshCaches();
}
```
