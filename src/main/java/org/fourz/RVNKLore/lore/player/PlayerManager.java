...
public class PlayerManager {
    ...
    public boolean createPlayerLoreEntry(Player player) {
        ...
        try {
            // Use synchronized block to prevent race conditions when checking existence
            synchronized (this) {
                // Double-check that player doesn't already exist by checking both content and name patterns
                if (playerExists(player.getUniqueId())) {
                    logger.warning("Attempted to create player lore entry for existing player: " + player.getName());
                    return false;
                }
    ...
    public boolean createFirstJoinLoreEntry(Player player) {
        ...
        try {
            ...
            synchronized (this) {
                // Double-check that player doesn't already exist by checking both content and name patterns
                if (playerExists(player.getUniqueId())) {
                    logger.warning("Attempted to create first join lore entry for existing player: " + player.getName());
                    return false;
                }
                  // Create a guaranteed unique entry name with UUID
    ...
    public boolean createNameChangeLoreEntry(Player player, String oldName) {
        ...
        try {
            ...
            synchronized (this) {
                // Validate that this is actually a different name
                if (oldName.equals(player.getName())) {
                    logger.debug("Skipping name change entry - names are identical: " + player.getName());
                    return false;
                }
                ...
                entry.setDescription("The adventurer known as " + oldName + " shall henceforth be known as " + 
                                   player.getName() + ".\nName changed on " + dateString + ".");                entry.setLocation(player.getLocation());
                entry.setSubmittedBy("Server");
