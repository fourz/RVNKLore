        ...
        if (meta != null) {
            ...
            if (entry.getMetadata("founding_date") != null) {
                try {
                    ...
                    String formatted = DefaultLoreHandler.DATE_FMT.format(
                            Instant.ofEpochMilli(foundingTimestamp).atZone(ZoneId.systemDefault()).toLocalDate());
                    lore.add(ChatColor.GRAY + "Founded: " + ChatColor.WHITE + formatted);
    ...
    public void displayLore(LoreEntry entry, Player player) {
        ...
        if (entry.getMetadata("founding_date") != null) {
            try {
                ...
                String formatted = DefaultLoreHandler.DATE_FMT.format(
                        Instant.ofEpochMilli(foundingTimestamp).atZone(ZoneId.systemDefault()).toLocalDate());
                player.sendMessage(ChatColor.GRAY + "Founded: " + ChatColor.WHITE + formatted);
            } catch (NumberFormatException e) {
                logger.debug("Could not parse founding date for city: " + entry.getName());
            }
        }
