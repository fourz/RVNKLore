 ...
 */
public class TavernLoreHandler implements LoreHandler {
    private final RVNKLore plugin;
    private final LogManager logger;

    public TavernLoreHandler(RVNKLore plugin) {
        this.plugin = plugin;
        this.logger = LogManager.getInstance(plugin, "TavernLoreHandler");
    }
    ...
    public ItemStack createLoreItem(LoreEntry entry) {
        ...
        if (meta != null) {
            meta.setDisplayName(ChatColor.DARK_AQUA + entry.getName());

            List<String> lore = new ArrayList<>();
            lore.add(ChatColor.GRAY + "Type: " + ChatColor.DARK_AQUA + "Tavern");
    ...
    public void displayLore(LoreEntry entry, Player player) {
        player.sendMessage(ChatColor.DARK_AQUA + "==== " + entry.getName() + " ====");
        player.sendMessage(ChatColor.GRAY + "Type: " + ChatColor.DARK_AQUA + "Tavern");
