 ...
 */
public class MonumentLoreHandler implements LoreHandler {
    private final RVNKLore plugin;
    private final LogManager logger;

    public MonumentLoreHandler(RVNKLore plugin) {
        this.plugin = plugin;
        this.logger = LogManager.getInstance(plugin, "MonumentLoreHandler");
    }
    ...
    public ItemStack createLoreItem(LoreEntry entry) {
        ...
        if (meta != null) {
            ...
            lore.add(ChatColor.GRAY + "Type: " + ChatColor.GOLD + "Monument");

            if (entry.getSubmittedBy() != null) {
                lore.add(ChatColor.GRAY + "Erected by: " + ChatColor.YELLOW + entry.getSubmittedBy());
            }
    ...
    public void displayLore(LoreEntry entry, Player player) {
        ...
        player.sendMessage(ChatColor.GRAY + "Type: " + ChatColor.GOLD + "Monument");

        if (entry.getSubmittedBy() != null) {
            player.sendMessage(ChatColor.GRAY + "Erected by: " + ChatColor.YELLOW + entry.getSubmittedBy());
        }
